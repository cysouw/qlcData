...
pacific <- "+proj=longlat +lon_wrap=155 +lon_0=155 +ellps=WGS84 +datum=WGS84 +no_defs"
...
mollweide_atlantic <- "+proj=moll +lon_wrap=10 +lon_0=10 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs "
...
add <- sapply(which(!test),function(x){makePoly(as.numeric(coords[x,]), size = .1)},simplify=F)
...
poly <- spTransform(poly, CRS(mollweide_atlantic))
...
save(cart,v,langs,map,file="~/Dropbox/Desktop/cart5.Rdata")
